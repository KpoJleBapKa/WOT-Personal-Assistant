...
QVariantMap BehaviorAnalyzer::analyze(const QVariantMap &replayData, const QVariantMap &metrics)
{
    ...
    // ❗️ НОВИЙ ПУНКТ АНАЛІЗУ
    QString keySkill = "Не визначено";
    if (performanceGrade == "Еталонний" || performanceGrade == "Дуже хороший")
    {
        if (blockedDamage > totalDamage && vehicleType.contains("heavy", Qt::CaseInsensitive)) {
            keySkill = "Майстерне танкування";
        } else if (assistedDamage > totalDamage && (vehicleType.contains("light", Qt::CaseInsensitive) || vehicleType.contains("medium", Qt::CaseInsensitive))) {
            keySkill = "Ефективна розвідка";
        } else if (spotted >= 5 && assistedDamage > 1500) {
            keySkill = "Агресивний підсвіт";
        }
        else {
            keySkill = "Реалізація шкоди";
        }
    }
    analysis["keySkill"] = keySkill;
